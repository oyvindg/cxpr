...
static cxpr_token_type cxpr_check_keyword(const char* text, size_t length) {
    const cxpr_keyword_entry* kw;

    for (kw = cxpr_keywords; kw->keyword; kw++) {
...
static cxpr_token cxpr_lexer_identifier(cxpr_lexer* lexer) {
    ...
    if ((length == 4 && (memcmp(start, "null", 4) == 0 || memcmp(start, "NULL", 4) == 0)) ||
        ...
        tok = cxpr_make_token(CXPR_TOK_NUMBER, start, length, start_pos, start_line, start_col);
        tok.number_value = (double)NAN;
        return tok;
    }

    /* Check for keyword aliases */
    kw_type = cxpr_check_keyword(start, length);
...
cxpr_token cxpr_lexer_next(cxpr_lexer* lexer) {
    ...
    if (*lexer->current == '\0') {
        return cxpr_make_token(CXPR_TOK_EOF, lexer->current, 0, start_pos, start_line, start_col);
    }
    ...
    switch (c) {
        ...
        case '!':
            ...
            return cxpr_make_token(CXPR_TOK_NOT, start, 1, start_pos, start_line, start_col);
        case '=':
            if (*lexer->current == '=') {
                cxpr_lexer_advance(lexer);
                return cxpr_make_token(CXPR_TOK_EQ, start, 2, start_pos, start_line, start_col);
            }
            return cxpr_make_token(CXPR_TOK_ASSIGN, start, 1, start_pos, start_line, start_col);
        case '<':
            if (*lexer->current == '=') {
                cxpr_lexer_advance(lexer);
                return cxpr_make_token(CXPR_TOK_LTE, start, 2, start_pos, start_line, start_col);
            }
            return cxpr_make_token(CXPR_TOK_LT, start, 1, start_pos, start_line, start_col);
        case '>':
            if (*lexer->current == '=') {
                cxpr_lexer_advance(lexer);
                return cxpr_make_token(CXPR_TOK_GTE, start, 2, start_pos, start_line, start_col);
            }
            return cxpr_make_token(CXPR_TOK_GT, start, 1, start_pos, start_line, start_col);
        case '&':
            if (*lexer->current == '&') {
                cxpr_lexer_advance(lexer);
...
cxpr_token cxpr_lexer_peek(cxpr_lexer* lexer) {
    cxpr_lexer save = *lexer;
    return cxpr_lexer_next(&save);
}
