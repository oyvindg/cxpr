...
static void test_builtin_nan_inf(void) {
    ...
    assert(ast);
    result = cxpr_eval(ast, ctx, reg, &err);
    assert(err.code == CXPR_OK);
    assert(isinf(result));
...
static void test_complex_math(void) {
    ...
    assert(ast);
    ASSERT_DOUBLE_EQ(cxpr_eval(ast, ctx, reg, &err), 5.0);
    cxpr_ast_free(ast);
...
static void test_extreme_nested_expressions(void) {
    ...
    assert(ast);
    ASSERT_DOUBLE_EQ(cxpr_eval(ast, ctx, reg, &err), 8.0);
    cxpr_ast_free(ast);
    ...
    assert(ast);
    ASSERT_DOUBLE_EQ(cxpr_eval(ast, ctx, reg, &err), 10.0);
    cxpr_ast_free(ast);
    ...
    assert(ast);
    ASSERT_APPROX(cxpr_eval(ast, ctx, reg, &err), 1.0, 1e-12);
    cxpr_ast_free(ast);
    ...
    assert(ast);
    ASSERT_DOUBLE_EQ(cxpr_eval(ast, ctx, reg, &err), -1.0);
    cxpr_ast_free(ast);

    cxpr_context_set(ctx, "rsi", 50.0);
    ast = cxpr_parse(p, "rsi < $low ? -1.0 : (rsi > $high ? 1.0 : 0.0)", &err);
    assert(ast);
    ASSERT_DOUBLE_EQ(cxpr_eval(ast, ctx, reg, &err), 0.0);
    cxpr_ast_free(ast);
    ...
    ast = cxpr_parse(p,
        ...
        &err);
    assert(ast);
    assert(cxpr_eval_bool(ast, ctx, reg, &err) == true);
    cxpr_ast_free(ast);
    ...
    assert(ast);
    ASSERT_APPROX(cxpr_eval(ast, ctx, reg, &err), 101.4, 1e-10);
    cxpr_ast_free(ast);
    ...
    assert(ast);
    ASSERT_DOUBLE_EQ(cxpr_eval(ast, ctx, reg, &err), 3.0);
    cxpr_ast_free(ast);

    /* and the other root: x=2 */
    ast = cxpr_parse(p, "(-b - sqrt(b^2 - 4*a*c)) / (2*a)", &err);
    assert(ast);
    ASSERT_DOUBLE_EQ(cxpr_eval(ast, ctx, reg, &err), 2.0);
    cxpr_ast_free(ast);
    ...
    ast = cxpr_parse(p,
        ...
        &err);
    assert(ast);
    ASSERT_DOUBLE_EQ(cxpr_eval(ast, ctx, reg, &err), 9.0);
    cxpr_ast_free(ast);
