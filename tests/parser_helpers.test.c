...
cxpr_token cxpr_expr_parser_peek_next(const parser_test_state* p);
...
static void test_parser_helper_functions(void) {
    ...
    tok = cxpr_expr_parser_peek_next(&p);
    assert(tok.type == CXPR_TOK_PLUS);

    text = cxpr_expr_parser_token_to_string(&p.current);
    assert(text);
    assert(strcmp(text, "alpha") == 0);
    free(text);

    assert(parser);
    ast = cxpr_expr_ast_parse(parser, "fn(x=1, y=2)", &err);
    assert(ast);
    clone = cxpr_expr_parser_clone_ast(ast);
    assert(clone);
    assert(cxpr_expr_ast_kind_of(clone) == CXPR_NODE_FUNCTION_CALL);
    assert(cxpr_expr_ast_call_has_named_args(clone));
