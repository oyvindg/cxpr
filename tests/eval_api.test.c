...
static void test_eval_api_wrappers(void) {
    ...
    ast = cxpr_expr_ast_parse(p, "x > 3 ? 1 : 0", &err);
    assert(ast);
    ...
    ast = cxpr_expr_ast_parse(p, "x > 3", &err);
    assert(ast);
    ...
    cxpr_expr_ast_free(ast);
    ast = cxpr_expr_ast_parse(p, "\"1h\"", &err);
    assert(ast);
    assert(cxpr_eval_ast(ast, ctx, reg, &value, &err));
    ...
    ast = cxpr_expr_ast_parse(p, "\"1h\" == \"1h\"", &err);
    assert(ast);
    assert(cxpr_eval_ast_bool(ast, ctx, reg, &boolean, &err));
    assert(boolean);
    cxpr_expr_ast_free(ast);
    ...
    ast = cxpr_expr_ast_parse(p, "symbol == \"AAPL\"", &err);
    assert(ast);
    assert(cxpr_eval_ast_bool(ast, ctx, reg, &boolean, &err));
    assert(boolean);
    cxpr_expr_ast_free(ast);
    ...
    ast = cxpr_expr_ast_parse(p, "$tf == \"1h\"", &err);
    assert(ast);
    assert(cxpr_eval_ast_bool(ast, ctx, reg, &boolean, &err));
    assert(boolean);
    cxpr_expr_ast_free(ast);

    ast = cxpr_expr_ast_parse(p, "1 + (x > 3)", &err);
    assert(ast);
    assert(!cxpr_eval_ast(ast, ctx, reg, &value, &err));
    assert(err.code == CXPR_ERR_TYPE_MISMATCH);
    cxpr_expr_ast_free(ast);
