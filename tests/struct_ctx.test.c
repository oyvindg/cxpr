...
static void test_double_field(void) {
    ...
    bool found = false;
    cxpr_field_value fv = cxpr_context_get_field(ctx, "pos", "x", &found);
    assert(found);
    assert(fv.type == CXPR_FIELD_DOUBLE);
    ASSERT_DOUBLE_EQ(fv.d, 1.0);

    cxpr_field_value r = eval_typed("pos.x + pos.y", ctx, reg);
...
static void test_bool_field(void) {
    ...
    cxpr_field_value vals[] = {cxpr_fv_bool(true), cxpr_fv_double(5.0)};
    cxpr_struct_value *s = cxpr_struct_value_new(names, vals, 2);
    cxpr_context_set_struct(ctx, "sensor", s);
    cxpr_struct_value_free(s);

    bool found = false;
    cxpr_field_value fv = cxpr_context_get_field(ctx, "sensor", "active", &found);
    assert(found);
    assert(fv.type == CXPR_FIELD_BOOL);
    assert(fv.b == true);

    /* sensor.active (bool) && x > 0.0 (bool) → bool */
    cxpr_context_set(ctx, "x", 1.0);
    cxpr_field_value r = eval_typed("sensor.active && x > 0.0", ctx, reg);
...
static void test_nested_struct_field(void) {
    ...
    cxpr_field_value inner_vals[] = {cxpr_fv_double(9.0)};
    ...
    bool found = false;
    cxpr_field_value fv = cxpr_context_get_field(ctx, "outer", "inner", &found);
    assert(found);
    assert(fv.type == CXPR_FIELD_STRUCT);
...
static void test_context_clear(void) {
    ...
    const char *names[] = {"v"};
    cxpr_field_value vals[] = {cxpr_fv_double(1.0)};
...
static void test_context_clone(void) {
    ...
    const char *names[] = {"v"};
    cxpr_field_value vals[] = {cxpr_fv_double(5.0)};
    cxpr_struct_value *s = cxpr_struct_value_new(names, vals, 1);
    cxpr_context_set_struct(ctx, "obj", s);
    cxpr_struct_value_free(s);
    ...
    bool found = false;
    cxpr_field_value fv = cxpr_context_get_field(clone, "obj", "v", &found);
    assert(found);
...
static void test_parent_walk(void) {
    ...
    bool found = false;
    cxpr_field_value fv = cxpr_context_get_field(child, "obj", "v", &found);
    assert(found);
    ASSERT_DOUBLE_EQ(fv.d, 42.0);

    /* expression via child context also resolves from parent */
    cxpr_field_value r = eval_typed("obj.v", child, reg);
    assert(r.type == CXPR_FIELD_DOUBLE);
