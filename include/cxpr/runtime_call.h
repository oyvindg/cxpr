 ...
 * numeric arguments, and optional scope values to their runtime backends.
 */
...
typedef struct {
    ...
    const char* scope_value;     /**< Optional scope value, such as timeframe `1d` or warehouse `warehouse-a`. */
    size_t arg_count;            /**< Raw AST argument count. */
    size_t value_arg_count;      /**< Numeric/value argument count excluding scope args. */
} cxpr_runtime_call;
 ...
 *
 * This recognizes trailing string scope arguments positionally, for example
 ...
 * @param[in] provider Optional provider used to resolve named scope args.
 * @param[in] ast Function-call or producer-access AST node.
 * @param[out] out Receives the decoded call view on success.
 * @return Non-zero when @p ast is a supported runtime call node.
 *
 * With provider metadata, named scope arguments such as
 * `close(timeframe="1d")` are exposed through @ref cxpr_runtime_call::scope_value
 ...
 * @return Non-zero when a provider-declared scope argument was found.
 */
